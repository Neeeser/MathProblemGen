...
export async function GET(request: Request, { params }: { params: { grade: string, topic: string, subTopic: string, questionType: string } }) {
    await dbConnect();
    ...
    try {
        ...
        return NextResponse.json({ success: true, data: questionTypeDoc }, { status: 200 });
    } catch (error) {
